...
class TransferManager private constructor(private val context: Context) {
  ...
  private var bleCommunicationSetup: BleEngagementSetup? = null
  ...
  fun startBleEngagement() {
//        if (hasStarted) {
//            throw IllegalStateException("Transfer has already started.")
//        }

    val bluetoothManager = context.getSystemService(BluetoothManager::class.java)
    bleCommunicationSetup = BleEngagementSetup(
      context,
      bluetoothManager,
      SessionSetup(CredentialStore(context)).createSession(),
      onConnecting = {
        transferStatusLd.postValue(TransferStatus.CONNECTING)
      },
      onConnected = {
        transferStatusLd.postValue(TransferStatus.CONNECTED)
      },
      onDisconnected = {
        transferStatusLd.postValue(TransferStatus.DISCONNECTED)
      },
      onError = { error ->
        log("onError: ${error.message}")
        transferStatusLd.postValue(TransferStatus.ERROR)
      }).apply { connectAsMdocHolder() }
  }
  ...
  fun disconnect() {
    ...
    bleCommunicationSetup?.disconnect()
